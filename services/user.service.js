	...
	return new Promise(async (resolve, reject) => {
		...
		if (!user)
			reject({
				status: "error",
				code: 404,
				message: `${_.capitalize(filterQuery.role) || "User"} not found`,
			})
		resolve(user)
	})
}
...
export const getAllStudentsByClass = async (classId, pageFilter) => {
	...
	if (!pageFilter || (!pageFilter.page && !pageFilter.limit)) return await User.find(findFilter)

	pageFilter.customLabels = paginationLabel

	return await User.paginate(findFilter, pageFilter)
}

export const getMany = async (filterQuery, pageFilter) => {
	if (!pageFilter || (!pageFilter.page && !pageFilter.limit)) return await User.find(filterQuery)
...
export const createUser = (data, file) => {
	return new Promise(async (resolve, reject) => {
		...
		if (data.role === "Staff" || data.role === "Admin") {
			...
			let theClass
			try {
				if (!_.isUndefined(data.classId) && isValidObjectId(data.classId))
					theClass = await classService.getOneClass({ _id: data.classId })
			} catch (error) {
				reject(error)
			}
...
export const updateUser = async (user, data, file) => {
	...
	if (file)
		image = user.profileImage
			? await uploadToCloudinary(file, user.profileImage.imageId)
			: await uploadToCloudinary(file)

	user.fullName = data.fullName || user.fullName
	user.profileImage = image
		? { url: image.secure_url, imageId: image.public_id }
	...
	if (user.role === "Staff") {
		...
		if (data.classId) user.class = await classService.getOneClass({ _id: data.classId })
