...
import { sendEmail } from '../mailers/passwordResetEmail.js'
...
export const validateOtp = async (user, code) => {
    const now = new Date();
    if (code !== user.resetPasswordToken) throw({ status: "error", code: 400, message: "Invalid otp code" })
    if (user.resetTokenExpiry < now) throw({ status: "error", code: 400, message: "Otp code expired" })
...
export const handleResetPassword = async (user, newPassword) => {
    ...
    await sendEmail(user, 'resetSuccess')
