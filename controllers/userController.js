...
export const batchCreateUser = async (req, res, next) => {
	...
	if (_.isEmpty(result.newUsers)) {
		throw {
			status: "error",
			code: 422,
			message: { content: "No users record was created", errors: result.errors },
		}
		...
		next({
			...
			data: {
				...
				message: `${result.newUsers.length} new users created, and ${result.errors.length} users failed to create`,
			},
		})
	} else {
		next({
			status: "success",
			data: { message: `${result.newUsers.length} new users created` },
		})
	}
}
...
export const assignClass = async (req, res, next) => {
	...
	next({
		status: "success",
		data: { message: `Successfully assigned to ${theClass.title}`, details: updatedClass },
	})
}
...
export const assignSubject = async (req, res, next) => {
	...
	next({
		status: "success",
		data: { message: `Successfully assigned to ${subject.title}`, details: updatedSubject },
	})
}
